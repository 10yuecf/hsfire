...
@property (nonatomic, strong) BMKPointAnnotation *pointWz;
...
- (void)buttonTap:(UIButton *)button {
    ...
    if (button.tag == 3) {
        ...
        _sytype = @"3";
        [self.mapView removeAnnotations:self.mapView.annotations];
        [self loadData:_lngcnow Lat:_latcnow Sytype:@"3"];
    }
}
...
- (BMKAnnotationView *)mapView:(BMKMapView *)mapView viewForAnnotation:(id <BMKAnnotation>)annotation {
    if (annotation == (BMKPointAnnotation *)_pointA) {
        ...
        NSString *AnnotationViewID = @"jydwmark";
    ...
    else if (annotation == (BMKPointAnnotation *)_pointB) {
        
        NSString *AnnotationViewID = @"tzclmark";
        BMKPinAnnotationView *annotationView = (BMKPinAnnotationView *)[mapView dequeueReusableAnnotationViewWithIdentifier:AnnotationViewID];
        if (annotationView == nil) {
            annotationView = [[BMKPinAnnotationView alloc] initWithAnnotation:annotation reuseIdentifier:AnnotationViewID];
            // 设置颜色
            ...
            annotationView.draggable = YES;
            // 把大头针换成别的图片
            annotationView.image = [UIImage imageNamed:@"jy2"];
        }
        return annotationView;
    }
    if (annotation == (BMKPointAnnotation *)_pointC) {
        
        NSString *AnnotationViewID = @"jywzmark";
        BMKPinAnnotationView *annotationView = (BMKPinAnnotationView *)[mapView dequeueReusableAnnotationViewWithIdentifier:AnnotationViewID];
        if (annotationView == nil) {
            annotationView = [[BMKPinAnnotationView alloc] initWithAnnotation:annotation reuseIdentifier:AnnotationViewID];
            // 设置颜色
            ...
            annotationView.draggable = YES;
            // 把大头针换成别的图片
            annotationView.image = [UIImage imageNamed:@"jy3"];
        }
        return annotationView;
    }
...
- (void)mapView:(BMKMapView *)mapView annotationViewForBubble:(BMKAnnotationView *)view {
    ...
    if([tt.title isEqualToString:@"我的位置"]) {
        ...
        NSString *url = [NSString stringWithFormat:@"%@index.php/Home/Index/jyinfo/xf_dt/",URL_IMG];
...
-(void)loadData:(NSString*)lng Lat:(NSString*)lat Sytype:(NSString*) sytype {
    ...
    NSDictionary *parameter = @{@"lng":lng,
                                @"lat":lat,
                                @"sytype":sytype};
    [CKHttpCommunicate createRequest:GetJyList WithParam:parameter withMethod:POST success:^(id response) {
        //NSLog(@"%@",response);
...
//                        _pointXfs.title = [NSString stringWithFormat:@"%@",array[i][@"dw_name"]];
//                        _pointXfs.subtitle = [NSString stringWithFormat:@"%@",array[i][@""]];
//                        [_mapView addAnnotation:_pointXfs];
                        
                        _pointA = [[FirstPointAnnotation alloc] initWithLatitude:dlat andLongtude:dlng];
                        _pointA.title = [NSString stringWithFormat:@"%@",array[i][@"dw_name"]];
                        _pointA.subtitle = [NSString stringWithFormat:@"%@",array[i][@"dw_adder"]];
...
//                        _pointSy.title = [NSString stringWithFormat:@"%@",array[i][@"cl_type"]];
//                        _pointSy.subtitle = [NSString stringWithFormat:@"%@",array[i][@"cl_adder"]];
//                        [_mapView addAnnotation:_pointSy];
                        
                        _pointB = [[FirstPointAnnotation alloc] initWithLatitude:dlat andLongtude:dlng];
                        _pointB.title = [NSString stringWithFormat:@"%@",array[i][@"cl_type"]];
                        _pointB.subtitle = [NSString stringWithFormat:@"%@",array[i][@"cl_adder"]];
...
//                        _pointWz.title = [NSString stringWithFormat:@"%@",array[i][@"wz_type"]];
//                        _pointWz.subtitle = [NSString stringWithFormat:@"%@",array[i][@"adder"]];
//                        [_mapView addAnnotation:_pointWz];
                        
                        _pointC = [[FirstPointAnnotation alloc] initWithLatitude:dlat andLongtude:dlng];
                        _pointC.title = [NSString stringWithFormat:@"%@",array[i][@"wz_type"]];
                        _pointC.subtitle = [NSString stringWithFormat:@"%@",array[i][@"adder"]];
