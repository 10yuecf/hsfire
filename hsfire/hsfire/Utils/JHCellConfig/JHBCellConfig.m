...
{
    JHBCellConfig *cellConfig = [JHBCellConfig new];
...
- (UITableViewCell *)cellOfCellConfigWithTableView:(UITableView *)tableView dataModel:(id)dataModel dataTitle:(id)dataTtile {
    return [self cellOfCellConfigWithTableView:tableView dataModel:dataModel dataTitle:dataTtile isNib:NO];
}

/// 根据cellConfig生成cell，重用ID为cell类名,可使用Nib
- (UITableViewCell *)cellOfCellConfigWithTableView:(UITableView *)tableView dataModel:(id)dataModel dataTitle:(id)dataTtile isNib:(BOOL)isNib {
    ...
    if (self.showInfoMethod && [cell respondsToSelector:self.showInfoMethod]) {
        ...
        [cell performSelector:self.showInfoMethod withObject:dataModel withObject:dataTtile];
