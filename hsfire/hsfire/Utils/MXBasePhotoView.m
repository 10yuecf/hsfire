...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        //NSLog(@"_deleBt======%f",frame.size.width);
...
- (void)delePhoto:(UIButton *)button {
    //NSLog(@"delete button tag %ld",(long)button.tag);
    if (_photoDelegate && [_photoDelegate respondsToSelector:@selector(clickDeleBtToDelePhotoWithView:BtnTag:)]) {
        NSString *btntag = [NSString stringWithFormat:@"%ld",(long)button.tag];
        [_photoDelegate clickDeleBtToDelePhotoWithView:button.superview BtnTag:btntag];
    }
}
